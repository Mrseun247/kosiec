...
// Edit an existing admin/staff account (name, email, role, and
// optionally reset the password) — super_admin only
// ============================================================
router.put(
  '/users/:id',
  protect,
  ...
  async (req, res, next) => {
    ...
    try {
      const { fullName, email, role, phone, staffId, password } = req.body;
      ...
      const before = `${user.fullName} (${user.email}), role: ${user.role}`;
      user.fullName = fullName;
      user.email = email;
      user.role = role;
      user.phone = phone;
      user.staffId = staffId || undefined;
      // Only reset the password if a new one was actually provided —
      // the pre-save hook re-hashes it automatically.
      ...
      await writeLog({
        ...
        details: `Edited account. Before: ${before} → After: ${user.fullName} (${user.email}), role: ${user.role}.${password ? ' Password was reset.' : ''}`,
        ipAddress: req.ip, userAgent: req.headers['user-agent'],
      });
